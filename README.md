...
> **HarmonyOS 蒙版调色原型** · Mask & Seal 设计语言 · ArkTS / ArkUI

Vivid 是一款运行在 HarmonyOS 上的图像调色与装裱工具原型。用户从相册选取一张照片，经过调色、滤镜、文字、边框四道工序，最终合成并导出到相册。全程离线运算，图像不离开设备。
...
| 功能 | 说明 |
|---|---|
| 📷 从相册导入 | 系统 PhotoViewPicker 单图选取，稳定沙箱 URI |
| 🎨 LUT 滤镜 | 16³ / 33³ `.cube`，Vulkan GPU 优先，CPU 回退 |
| ✍️ 文字图层 | 最多 8 层，5 种字体，8 种颜色，实时预览 |
| 🖼 边框 / 水印 | 23 种模板（素白 / 胶片 / 宣纸印章等），含相机 EXIF 水印 |
| 💾 作品存档 | 已封印 / 草稿分类，本地持久化，支持续编 |
| 📤 导出到相册 | 可选 JPEG / PNG / HEIF，分辨率 1080 p / 1440 p / 原图 |
| 👤 账号（Mock） | 验证码 + 密码登录，换绑手机号，注销闭环 |
...
- 作品列表与详情仍使用 MockWorks；影像调色页已接入图片预览、文字/边框合成和导出流程，但尚未接入云端保存。
...
| 影像调色 | pages/EditPage | 图片预览、LUT 滤镜、文字/边框编辑、强度调节与导出 |
