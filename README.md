...
Vivid 是一个基于 HarmonyOS ArkTS 的「蒙版调色」前端界面原型。当前版本为 `0.1.1`，聚焦“我的”及其登录、注册、设置和协议页面，提供页面跳转、表单交互、前端校验与响应式布局示例。
...
| 应用设置 | `pages/AppSettingsPage` | 照片导出质量与格式、语言和本地说明入口 |
...
- 未登录用户设置卡片可跳转到登录页。
- 用户设置页展示个人册页与个人称谓入口；应用设置页提供标准、高清、超清的照片导出质量，以及 JPEG（默认）和 PNG 格式选项。
...
- 照片导出质量与格式选项目前仅用于前端展示，尚未接入真实导出功能。
